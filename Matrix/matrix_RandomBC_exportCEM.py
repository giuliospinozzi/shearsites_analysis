...
# output configs
out_filename = "exportCEM.tsv"  # path is build by matrix_RandomBC_outputModule.buildOutputPath
#                                 with parameters from matrix_RandomBC_globModule
sep = '\t'
eol = '\n'
# misc
...
def relabelling(df, asso_dict):
    return matrix_RandomBC_outputModule.relabelling(df, asso_dict, use_fields=(3,2,4,7,8), concat=relabelling_sep, inplace=False)
...
def exportCEM(df_dict, asso_dict, filename=out_filename, sep=sep, eol=eol):
    ...
    with open(outpath, 'w') as out_stream:
        ...
        out_stream.write(eol.join(rows))
