...
def trova_punto (nome_file_sorgente, elenco_picchi_CEM): # nome_file_sorgente da elenco_sorgenti
	...
	file_outcomes_dictionary = {} # {genome_location_string : relative_abundance_float}
	for line in file_outcomes_lines[1:]:
		line_split = line.split('\t')
		key = line_split[0] + " " + line_split[1] + " " + line_split[2]
		relative_abundance = float(line_split[4])
		file_outcomes_dictionary.update({key:relative_abundance})
	...
	for cem in elenco_picchi_CEM:
		...
		if (file_outcomes_dictionary.has_key(genome_location)):
			relative_abundance = relative_abundance + file_outcomes_dictionary[genome_location]
		...
		if (file_outcomes_dictionary.has_key(genome_location_back)):
			relative_abundance = relative_abundance + file_outcomes_dictionary[genome_location_back]
		if (file_outcomes_dictionary.has_key(genome_location_forward)):
			relative_abundance = relative_abundance + file_outcomes_dictionary[genome_location_forward]
		cem_dictionary.update({genome_location:relative_abundance})
...
#File output
#file_output = open ('gatc_plot.txt', 'w') #Da cambiare
file_output = open ('gsk_plot.txt', 'w')
